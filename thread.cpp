...
namespace bgcc {
    ...
    bool Thread::join() {
        ...
        if (!pthread_equal(pthread_self(), _thread)) {//不要等待自己
            if (!_detached) {
    ...
    void* Thread::thread_start_func(void *arg) {
        ...
        RunnableSharedPointer runner = me->_functor; //智能指针引用计数
        bool use_functor = me->_use_functor;         //bool
        run_func_t func = me->_func_ptr;             //用户函数
        void* func_arg = me->_func_arg;              //参数 
        me->_sema.signal();                          //singal
